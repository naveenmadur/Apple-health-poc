...
class HealthAppState extends State<HealthApp> {
  ...
  int _nofSteps = 0;
  List<RecordingMethod> recordingMethodsToFilter = <RecordingMethod>[];
  ...
  List<HealthDataAccess> get permissions =>
      types.map((HealthDataType e) => HealthDataAccess.READ).toList();

  // Or both READ and WRITE
  // List<HealthDataAccess> get permissions => types.map(
  //       (HealthDataType type) {
  //         final bool containsNonWriteableData = <HealthDataType>[
  ...
  //         debugPrint(
  //           'Contains Non Writeable data: $containsNonWriteableData - ${type.name}',
  //         );

  //         return containsNonWriteableData
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      healthData.sort(
        (HealthDataPoint a, HealthDataPoint b) => b.dateTo.compareTo(a.dateTo),
      );
    ...
    for (final HealthDataPoint data in _healthDataList) {
  ...
  Future<void> addData() async {
    final DateTime now = DateTime.now();
    final DateTime earlier = now.subtract(const Duration(minutes: 20));
  ...
  Future<void> deleteData() async {
    final DateTime now = DateTime.now();
    final DateTime earlier = now.subtract(const Duration(hours: 24));
  ...
  Future<void> fetchStepData() async {
    ...
    bool stepsPermission =
        await Health().hasPermissions(<HealthDataType>[HealthDataType.STEPS]) ??
            false;
    if (!stepsPermission) {
      stepsPermission = await Health()
          .requestAuthorization(<HealthDataType>[HealthDataType.STEPS]);
    }
  ...
  Widget get _dataFiltration => Column(
        children: <Widget>[
          Wrap(
            children: <Widget>[
              for (final RecordingMethod method in Platform.isAndroid
                  ? <RecordingMethod>[
  ...
  Widget get _stepsFiltration => Column(
        children: <Widget>[
          Wrap(
            children: <Widget>[
              for (final RecordingMethod method in <RecordingMethod>[
                RecordingMethod.manual,
              ])
                SizedBox(
                  width: 150,
  ...
  String getDate(final DateTime date) {
  ...
  Widget get _contentDataReady => ListView.builder(
        ...
        itemBuilder: (_, int index) {
          ...
          final HealthDataPoint p = _healthDataList[index];
